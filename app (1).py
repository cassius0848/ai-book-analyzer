...
#  PRISM UI — 深黑终端风格
# ================================================================
# Check if any API key is in secrets
_has_secret_key = False
try:
    for _sk in ["my_deepseek_key","my_openai_key","my_anthropic_key","my_gemini_key"]:
        if st.secrets.get(_sk):
            _has_secret_key = True; break
except Exception:
    pass
...
with hdr_l:
    ...
    <div>
        ...
        <div class="ctx-subtitle">文件光谱分析 — 上传 · 解构 · 洞察 · 掌握</div>
    </div>
    """, unsafe_allow_html=True)
with hdr_r:
    lang = st.selectbox("LANG", ["中文", "English"], label_visibility="collapsed")
    st.markdown("""
...
st.markdown('<div style="border-bottom:1px solid var(--grid);margin-bottom:16px;"></div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    api_key = None
    try:
        api_key = st.secrets[prov_cfg["secret_key"]]
        st.caption(f"API: {provider} CONNECTED")
    except Exception:
        api_key = st.text_input("API_KEY", type="password",
                                help=f"输入 {provider} 的 API Key")

    base_url = st.text_input("BASE_URL", value=prov_cfg["base_url"])
    model_name = st.selectbox("MODEL", prov_cfg["models"])

    # 当切换 provider/model 时重置 LLM 缓存
    cache_key = f"{provider}_{model_name}_{base_url}"
    if st.session_state.get("_llm_cache_key") != cache_key:
...
■ 输出：只输出严格合法的 JSON 数组，直接 [ 开头 ] 结尾，不要其他文字。
【致命要求】：
1. 所有的属性名和字符串值必须使用双引号 (")，绝不能使用单引号。
2. 字符串内部如果需要出现双引号，必须使用反斜杠转义 (\")。
3. 字符串内部禁止出现真实的物理换行，全部用转义字符 \n 代替！
...
def get_lang_instruction():
    """Return language instruction string based on user selection"""
    if lang == "English":
        return "Write entirely in English. Use English for all explanations, definitions, examples. Keep technical terms in their standard English form."
    return "整体中文撰写，专业术语用「中文名 (English Term)」格式。"

def extract_single(text, query, llm, bl=None):
    bn = f"（第{bl}部分）" if bl else ""
    li = get_lang_instruction()
    resp = llm.invoke(PHASE1_PROMPT.format(query=query, batch_note=bn, text=text, lang_instruction=li)).content
    p = robust_json(resp)
...
def generate_synthesis(kps, query, llm):
    ...
    li = get_lang_instruction()
    resp = llm.invoke(SYNTHESIS_PROMPT.format(query=query, kps_json=kps_json, lang_instruction=li)).content
    return resp
